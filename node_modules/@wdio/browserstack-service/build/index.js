...
var EVENTS, TESTHUB_EVENTS, AUTOMATE_EVENTS, A11Y_EVENTS, PERCY_EVENTS, O11Y_EVENTS, HOOK_EVENTS, TURBOSCALE_EVENTS, APP_AUTOMATE_EVENTS, DRIVER_EVENT;
...
async function uploadLogs(user, key, clientBuildUuid) {
  try {
    ...
    await new Promise((resolve, reject) => {
      const source = fs4.createReadStream(tarPath);
      const dest = fs4.createWriteStream(tarGzPath);
    ...
    fs4.unlinkSync(tarPath);
    fs4.unlinkSync(tarGzPath);
    ...
    if (fs4.existsSync(CLI_DEBUG_LOGS_FILE)) {
      fs4.unlinkSync(CLI_DEBUG_LOGS_FILE);
    }
    return response;
  } catch (error) {
...
function removeDir(dir) {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    const stat = fs4.statSync(filename);
    if (filename === "." || filename === "..") {
    } else if (stat.isDirectory()) {
      removeDir(filename);
    } else {
      fs4.unlinkSync(filename);
    }
  }
  fs4.rmdirSync(dir);
}
...
function isWritable(dirPath) {
  try {
    fs4.accessSync(dirPath, fs4.constants.W_OK);
    return true;
  } catch {
    return false;
  }
...
var init_util = __esm({
  "src/util.ts"() {
    ...
    init_fetchWrapper();
    pGitconfig = promisify(gitconfig);
...
var init_bstackLogger = __esm({
  "src/bstackLogger.ts"() {
    ...
    init_constants();
    init_util();
    log = logger("@wdio/browserstack-service");
    BStackLogger = class {
  ...
  "src/Percy/PercyLogger.ts"() {
    ...
    PercyLogger = class {
      ...
      static logToFile(logMessage, logLevel) {
        try {
          if (!this.logFileStream) {
            if (!fs6.existsSync(this.logFolderPath)) {
              fs6.mkdirSync(this.logFolderPath);
            }
            this.logFileStream = fs6.createWriteStream(this.logFilePath, { flags: "a" });
          }
          if (this.logFileStream && this.logFileStream.writable) {
            this.logFileStream.write(this.formatLog(logMessage, logLevel));
          }
...
`;
      ...
      static clearLogFile() {
        if (fs6.existsSync(this.logFilePath)) {
          fs6.truncateSync(this.logFilePath);
        }
      }
    };
  }
...
var log3, BStackLogger2;
var init_cliLogger = __esm({
  "src/cli/cliLogger.ts"() {
    "use strict";
    init_constants();
    init_util();
    log3 = logger3("@wdio/browserstack-service/cli");
    BStackLogger2 = class {
      ...
      static logToFile(logMessage, logLevel) {
        try {
          if (!this.logFileStream) {
            this.ensureLogsFolder();
            this.logFileStream = fs11.createWriteStream(this.logFilePath, { flags: "a" });
          }
          if (this.logFileStream && this.logFileStream.writable) {
            this.logFileStream.write(this.formatLog(logMessage, logLevel));
          }
...
`;
      ...
      static clearLogFile() {
        if (fs11.existsSync(this.logFilePath)) {
          fs11.truncateSync(this.logFilePath);
        }
      }
      static ensureLogsFolder() {
        if (!fs11.existsSync(this.logFolderPath)) {
          fs11.mkdirSync(this.logFolderPath);
        }
      }
    };
  }
    ...
    CLIUtils = class _CLIUtils {
      ...
      static getWritableDir() {
        ...
        for (const path21 of writableDirOptions) {
          ...
          try {
            if (fs12.existsSync(path21)) {
              BStackLogger2.debug(`File ${path21} already exist`);
              if (!isWritable(path21)) {
                BStackLogger2.debug(`Giving write permission to ${path21}`);
                const success = setReadWriteAccess(path21);
                if (!isTrue(success)) {
                  BStackLogger2.warn(`Unable to provide write permission to ${path21}`);
                }
              }
            } else {
              BStackLogger2.debug(`File does not exist: ${path21}`);
              createDir(path21);
              BStackLogger2.debug(`Giving write permission to ${path21}`);
              const success = setReadWriteAccess(path21);
              if (!isTrue(success)) {
      ...
      static getExistingCliPath(cliDir) {
        try {
          if (!fs12.existsSync(cliDir) || !fs12.statSync(cliDir).isDirectory()) {
            return "";
          }
          const allBinaries = fs12.readdirSync(cliDir).map((file) => path11.join(cliDir, file)).filter((filePath) => fs12.statSync(filePath).isFile() && path11.basename(filePath).startsWith("binary-"));
          if (allBinaries.length > 0) {
            const latestBinary = allBinaries.map((filePath) => ({
              filePath,
              mtime: fs12.statSync(filePath).mtime
      ...
      static downloadLatestBinary = async (binDownloadUrl, cliDir) => {
        ...
        try {
          ...
          const downloadedFileStream = fs12.createWriteStream(zipFilePath);
          return new Promise((resolve, reject) => {
          ...
          try {
            ...
            zipfile.on("entry", async (entry) => {
              ...
              if (/\/$/.test(entry.fileName)) {
                ...
                const writeStream = fs12.createWriteStream(path11.join(cliDir, entry.fileName));
...
var PercyBinary = class {
  ...
  async #makePath(path21) {
    ...
    return fsp.mkdir(path21).then(() => true).catch(() => false);
  }
  async #checkPath(path21) {
    try {
      const hasDir = await fsp.access(path21).then(() => true, () => false);
      if (hasDir) {
        return true;
      }
    } catch {
  ...
  async download(destParentDir, response) {
    ...
    const downloadedFileStream = fs7.createWriteStream(zipFilePath);
    if (!response) {
      ...
      yauzl.open(zipFilePath, { lazyEntries: true }, function(err, zipfile) {
        ...
        zipfile.on("entry", (entry) => {
          if (/\/$/.test(entry.fileName)) {
            ...
            const writeStream = fs7.createWriteStream(
        ...
        zipfile.once("end", () => {
          fs7.chmod(binaryPath, "0755", function(zipErr) {
            if (zipErr) {
              reject(zipErr);
            }
            resolve(binaryPath);
...
var Percy = class {
  ...
  async start() {
    ...
    const logStream = fs8.createWriteStream(this.#logfile, { flags: "a" });
...
function getDataFromWorkers() {
  const workersData = [];
  if (!fs9.existsSync(workersDataDirPath)) {
    return workersData;
  }
  const files = fs9.readdirSync(workersDataDirPath);
  files.forEach((file) => {
...
function removeWorkersDataDir() {
  fs9.rmSync(workersDataDirPath, { recursive: true, force: true });
  return true;
}
function createWorkersDataDir() {
  if (!fs9.existsSync(workersDataDirPath)) {
    fs9.mkdirSync(workersDataDirPath, { recursive: true });
  }
  return true;
}
...
function saveFunnelData(eventType, config) {
  ...
  fs10.writeFileSync(filePath, JSON.stringify(data));
  return filePath;
}
